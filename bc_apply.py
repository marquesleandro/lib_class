...
def b_bc(_npoints, _x, _y, _bc, _neumann_edges,_dirichlet_pts, _LHS0, _neighbors_nodes): #For Method 2 and 3
#def b_bc(_npoints, _x, _y, _bc, _neumann_edges,_dirichlet_pts, _LHS0): #For Method 1
 ...
 _LHS = sps.lil_matrix.copy(_LHS0)
...
def streamfunction_bc(_npoints, _x, _y, _bc, _neumann_edges,_dirichlet_pts, _LHS0, _neighbors_nodes):
 bc_neumann = np.zeros([_npoints,1], dtype = float) 
 bc_dirichlet = np.zeros([_npoints,1], dtype = float) 
 ibc = [] 
 bc_1 = np.zeros([_npoints,1], dtype = float) #For scipy array solve
